            ...
            {messages.map((msg: any, index: number) => (
              <div key={index} className={`flex items-start space-x-3 mb-6`}>
                ...
                <div className='p-1 rounded-lg flex-1 text-[14px]'>
                  <pre style={{ wordWrap: 'break-word', whiteSpace: 'pre-wrap', color: msg.error ? 'red': 'black', display: 'flex', flexDirection:'column', alignItems: 'start' }}>
                    {msg.content}
                    {(msg.error && messages.length-1 === index) && <RxReload style={{ marginLeft: '10px' }} onClick={() => onSend(msg.content)}/>}
                  </pre>
                </div>
              </div>
            ))}
