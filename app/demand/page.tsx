...
export default memo(() => {
  ...
  const streamStepRef = useRef(null);
      ...
      await fetchEventSource(
        'https://api.indieapp.ai/v1/projects/ai/chat',
        {
          ...
          onmessage({ event, data }: any) {
            if (event === 'error') {
              Toast.notify({ type: 'error', message: data });
              return;
            }

            const str = data.replace(/data:\s*/g, '');
            // 标记接收标记，不做任何处理
            if (['#fls#', '#dcs#', '#end#'].includes(str)) {
              streamStepRef.current = str;
              return;
            }

            // 说明 features 生成了
            if (streamStepRef.current === '#fls#') {
              setState({
                showFeatures: true,
                features: JSON.parse(str || '[]').map((feature: any, index: number) => ({ id: index, ...feature })) 
              });
              return;
            }

            if (!streamStepRef.current) {
              handleSendMessage(_msgList, str, 'robot');
            }
          },
          onerror(err: any) {
            throw err
